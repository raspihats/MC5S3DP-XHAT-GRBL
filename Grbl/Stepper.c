...
void Stepper_WakeUp(void)
{
	...
	if(BIT_IS_TRUE(settings.flags, BITFLAG_INVERT_ST_ENABLE)) {
		LL_GPIO_SetOutputPin(GPIO_ENABLE_X_PORT, GPIO_ENABLE_X_PIN);
		LL_GPIO_SetOutputPin(GPIO_ENABLE_Y_PORT, GPIO_ENABLE_Y_PIN);
		LL_GPIO_SetOutputPin(GPIO_ENABLE_Z_PORT, GPIO_ENABLE_Z_PIN);
	}
	else {
		LL_GPIO_ResetOutputPin(GPIO_ENABLE_X_PORT, GPIO_ENABLE_X_PIN);
		LL_GPIO_ResetOutputPin(GPIO_ENABLE_Y_PORT, GPIO_ENABLE_Y_PIN);
		LL_GPIO_ResetOutputPin(GPIO_ENABLE_Z_PORT, GPIO_ENABLE_Z_PIN);
	}
...
void Stepper_MainISR(void)
{
    if(st.step_outbits & (1<<X_STEP_BIT)) {
		if(step_port_invert_mask & (1<<X_STEP_BIT)) {
			// Low pulse
			LL_GPIO_ResetOutputPin(GPIO_STEP_X_PORT, GPIO_STEP_X_PIN);
		}
		else {
			// High pulse
			LL_GPIO_SetOutputPin(GPIO_STEP_X_PORT, GPIO_STEP_X_PIN);
		}
    }
    if(st.step_outbits & (1<<Y_STEP_BIT)) {
		if(step_port_invert_mask & (1<<Y_STEP_BIT)) {
			// Low pulse
			LL_GPIO_ResetOutputPin(GPIO_STEP_Y_PORT, GPIO_STEP_Y_PIN);
		}
		else {
			// High pulse
			LL_GPIO_SetOutputPin(GPIO_STEP_Y_PORT, GPIO_STEP_Y_PIN);
		}
    ...
    if(st.step_outbits & (1<<Z_STEP_BIT)) {
		if(step_port_invert_mask & (1<<Z_STEP_BIT)) {
			// Low pulse
			LL_GPIO_ResetOutputPin(GPIO_STEP_Z_PORT, GPIO_STEP_Z_PIN);
		}
		else {
			// High pulse
			LL_GPIO_SetOutputPin(GPIO_STEP_Z_PORT, GPIO_STEP_Z_PIN);
		}
    }
	...
	if(st.exec_segment == 0) {
		...
		if(segment_buffer_head != segment_buffer_tail) {
			...
			if(st.dir_outbits & (1<<X_DIRECTION_BIT)) {
				LL_GPIO_SetOutputPin(GPIO_DIR_X_PORT, GPIO_DIR_X_PIN);
			}
			else {
				LL_GPIO_ResetOutputPin(GPIO_DIR_X_PORT, GPIO_DIR_X_PIN);
			}
			if(st.dir_outbits & (1<<Y_DIRECTION_BIT)) {
				LL_GPIO_SetOutputPin(GPIO_DIR_Y_PORT, GPIO_DIR_Y_PIN);
			}
			else {
				LL_GPIO_ResetOutputPin(GPIO_DIR_Y_PORT, GPIO_DIR_Y_PIN);
			}
			if(st.dir_outbits & (1<<Z_DIRECTION_BIT)) {
				LL_GPIO_SetOutputPin(GPIO_DIR_Z_PORT, GPIO_DIR_Z_PIN);
			}
			else {
				LL_GPIO_ResetOutputPin(GPIO_DIR_Z_PORT, GPIO_DIR_Z_PIN);
			}
