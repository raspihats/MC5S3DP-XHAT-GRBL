...
uint8_t Limits_GetState(void)
{
	...
	limit_state = (LL_GPIO_IsInputPinSet(GPIO_LIM_X_PORT, GPIO_LIM_X_PIN)<<X_LIMIT_BIT);
	limit_state |= (LL_GPIO_IsInputPinSet(GPIO_LIM_Y_PORT, GPIO_LIM_Y_PIN)<<Y_LIMIT_BIT);
	limit_state |= (LL_GPIO_IsInputPinSet(GPIO_LIM_Z_PORT, GPIO_LIM_Z_PIN)<<Z_LIMIT_BIT);
